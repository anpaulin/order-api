...
class Module(environment: Environment, configuration: Configuration) extends AbstractModule {
  ...
  override def configure(): Unit = {
    ...
    auditLogType.toLowerCase match {
      case "memory" => bind(classOf[AuditLogRepository]).to(classOf[InMemoryAuditLogRepository])
      case "kafka"  => bind(classOf[AuditLogRepository]).to(classOf[KafkaAuditLogRepository])
      case "mysql"  => bind(classOf[AuditLogRepository]).to(classOf[MySqlAuditLogRepository])
      case "file"   => bind(classOf[AuditLogRepository]).to(classOf[FileAuditLogRepository])
      case other    => throw new IllegalArgumentException(s"Unknown audit log type '$other'. Valid options are: file, memory, kafka, mysql")
    }

    bind(classOf[OrderService]).to(classOf[InMemoryOrderService])
  }
}
